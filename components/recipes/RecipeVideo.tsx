...
import {
  ...
  updateRecipeYoutubeUrl,
  checkVideoStatus,
  touchRecipeVideoTimestamp,
    ...
    async function loadVideo() {
      ...
      if (recipe.youtube) {
        ...
        if (existingVideoId) {
          ...
          const lastChecked = recipe.videoLastChecked
            ? new Date(recipe.videoLastChecked).getTime()
            : 0;
          ...
          if (isStale) {
            ...
            console.log(
              `[RecipeVideo] Verifying stale video for "${recipe.name}" (last checked: ${recipe.videoLastChecked || "never"})`
            );
            try {
              ...
              if (isValid) {
                ...
                console.warn(
                  `[RecipeVideo] Video ${existingVideoId} is unavailable. Searching for replacement...`
                );
        ...
        console.log(
          `[RecipeVideo] No YouTube URL for "${recipe.name}". Will attempt search.`
        );
      }
      ...
      if (!searchAttempted || needsReplacement) {
        ...
        try {
          ...
          if (foundVideoId) {
            console.log(
              `[RecipeVideo] Found video ID for "${recipe.name}":`,
              foundVideoId
            );
