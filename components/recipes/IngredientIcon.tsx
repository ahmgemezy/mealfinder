...
export default function IngredientIcon({
  ...
  const getEmojiForIngredient = (name: string): string => {
    ...
    if (lowerName.includes("cinnamon") || lowerName.includes("spice"))
      return "🌿";
    if (
  ...
  if (imageError) {
    // Fallback: show emoji
    return (
      <div className="w-12 h-12 md:w-14 md:h-14 shrink-0 bg-linear-to-br from-primary-100 to-primary-200 rounded-xl p-1 md:p-1.5 shadow-sm border border-primary-300 flex items-center justify-center group-hover:scale-105 transition-transform">
  ...
  return (
    <div className="relative w-12 h-12 md:w-14 md:h-14 shrink-0 bg-white rounded-xl p-1 md:p-1.5 shadow-sm border border-border group-hover:scale-105 transition-transform">
      <Image
        src={getImageSource()}
        alt={ingredientName}
        fill
