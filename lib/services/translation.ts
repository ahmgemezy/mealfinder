// Translation library - aliased to stub on Cloudflare via turbopack.resolveAlias
...
export async function translateText(
  ...
  try {
    ...
    if (
      ...
      console.error(
        `Translation error (${targetLang}). Text len: ${text.length}. Error:`,
        error
      );
    }
    return text;
...
export async function translateRecipe(
  ...
  try {
    ...
    if (cached && !error && cached.instructions) {
      // Ensure incomplete translations (from list view) don't block full translation
    ...
    devLog.log(
      `[Translation] Cache miss for ${recipe.name} (${locale}). Translating...`
    );
    ...
    const [translatedTitle, translatedInstructions, translatedIngredientsText] =
      await Promise.all([
 ...
 */
export async function translateRecipesList(
  recipes: Recipe[],
  locale: string
): Promise<Recipe[]> {
  ...
  try {
    ...
    if (missingRecipes.length > 0) {
      devLog.log(
        `[Translation] List: Translating ${missingRecipes.length} titles for ${locale}`
      );
 ...
 */
export async function translateBlogPosts<T extends BlogPost | DBBlogPost>(
  posts: T[],
  locale: string
): Promise<T[]> {
  ...
  try {
    ...
    if (cachedList && !error) {
      cachedList.forEach(
        (row: { post_slug: string; title: string; excerpt: string }) => {
          if (row.title)
            cachedMap.set(row.post_slug, {
              title: row.title,
              excerpt: row.excerpt,
            });
        }
    ...
    if (missingPosts.length > 0) {
      devLog.log(
        `[Translation] Blog: Translating ${missingPosts.length} posts for ${locale}`
      );
        ...
        chunkedPosts.map(async (chunk) => {
          try {
            ...
            const translatedChunk = chunk.map((post, index) => {
              ...
              const tTitle =
                parts && parts.length >= 2 ? parts[0].trim() : post.title;
              const tExcerpt =
                parts && parts.length >= 2 ? parts[1].trim() : post.excerpt;
...
export async function translateFAQ(
  ...
  try {
    devLog.log(
      `[Translation] FAQ: Translating ${faq.length} items for ${locale}`
    );
 ...
 */
export async function translateBlogPostFull<T extends BlogPost | DBBlogPost>(
  post: T,
  locale: string
): Promise<T> {
  ...
  try {
    ...
    devLog.log(
      `[Translation] translating full blog post: ${post.title} (Cache Miss)`
    );
...
export async function translateSEOEnrichment(
  ...
  try {
    ...
    devLog.log(
      `[Translation] SEO Cache miss for ${enrichment.recipeId} (${locale}). Translating...`
    );
