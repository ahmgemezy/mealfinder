...
import {
  getRandomMealClient,
  getRandomMealWithFiltersClient,
} from "@/lib/api/client";
...
export default function SurpriseMeContent() {
  ...
  const generateMeal = async () => {
    ...
    try {
      let newRecipe: Recipe | null;

      if (filters.category || filters.area) {
        newRecipe = await getRandomMealWithFiltersClient({
          category: filters.category || undefined,
          area: filters.area || undefined,
        });
      } else {
        newRecipe = await getRandomMealClient();
      }

      if (!newRecipe) {
        throw new Error(t("noRecipeFound"));
      }
      ...
      <div className="container mx-auto px-4">
        <div className="max-w-4xl mx-auto">
          ...
          <div className="bg-card rounded-2xl p-6 shadow-soft mb-8">
            <h2 className="font-semibold text-lg mb-4">{t("filterTitle")}</h2>
            ...
            <Button
              ...
              className="w-full"
            >
              {recipe ? t("tryAnotherButton") : t("generateButton")}
            </Button>
          </div>
